...
def test_stage_list(mock_execute, runner):
...
@mock.patch(f"{STAGE_MANAGER}._execute_query")
def test_stage_get(mock_execute, runner):
...
@mock.patch(f"{STAGE_MANAGER}._execute_query")
def test_stage_get_default_path(mock_execute, runner):
...
@mock.patch(f"{STAGE_MANAGER}._execute_query")
def test_stage_put(mock_execute, runner):
    with TemporaryDirectory() as tmp_dir:
        result = runner.invoke_with_config(
            [
                "stage",
...
@mock.patch(f"{STAGE_MANAGER}._execute_query")
def test_stage_create(mock_execute, runner):
...
@mock.patch(f"{STAGE_MANAGER}._execute_query")
def test_stage_drop(mock_execute, runner):
...
@mock.patch(f"{STAGE_MANAGER}._execute_query")
def test_stage_remove(mock_execute, runner):
    result = runner.invoke_with_config(
