import pytest
from pydantic import ValidationError

from snowflake.cli.api.project.schemas.snowpark.argument import Argument
from tests.testing_utils.fixtures import argument_instance, callable_instance

# TODO:
# -test nice error message when update fails - this should be done by yaml loader, here we only check that error is raised
# -test if runtime is passed correctly to Callable


def test_if_fields_are_updated_correctly_by_assignment(argument_instance):
    argument_instance.name = "Baz"

    assert argument_instance.name == "Baz"


def test_if_optional_fields_set_to_none_are_correctly_updated_by_assignment(
    argument_instance
):
    assert argument_instance.default == None

    argument_instance.default = "Baz"

    assert argument_instance.default == "Baz"


def test_if_model_is_validated_on_update_by_assingment(argument_instance):
    with pytest.raises(ValidationError) as expected_error:
        argument_instance.name = 42

    assert (
        "Input should be a valid string [type=string_type, input_value=42, input_type=int]"
        in str(expected_error.value)
    )


def test_if_model_is_updated_correctly_from_dict(argument_instance):
    assert argument_instance.default == None
    update_dict = {"name": "Baz", "default": "Foo"}

    argument_instance.update_from_dict(update_dict)

    assert argument_instance.default == "Foo"
    assert argument_instance.name == "Baz"
    assert argument_instance.arg_type == "Bar"


def test_nested_fields_update(callable_instance, argument_instance):
    assert Argument(name='a', type='string') in callable_instance.signature

    update_dict = {"signature": [argument_instance.dict()]}

    callable_instance.update_from_dict(update_dict)

    assert argument_instance in callable_instance.signature
