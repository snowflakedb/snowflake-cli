...
def test_command_without_any_options(cli, custom_snapshot):
    result = cli(app_factory(SnowTyperFactory))(["simple_cmd", "--help"])
    assert result.output == custom_snapshot


def test_command_with_global_options(cli, custom_snapshot):
    result = cli(app_factory(SnowTyperFactory))(["cmd_with_global_options", "--help"])
    assert result.output == custom_snapshot


def test_command_with_connection_options(cli, custom_snapshot):
...
def test_enabled_command_is_visible(cli, custom_snapshot):
...
def test_enabled_command_is_not_visible(cli, custom_snapshot):
    global _ENABLED_FLAG
    _ENABLED_FLAG = False
    result = cli(app_factory(SnowTyperFactory))(["switchable_cmd", "--help"])
