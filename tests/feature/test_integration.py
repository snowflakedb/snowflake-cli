...
"""Offline library-contract tests: ``FeatureManager`` wired to the real
``decl_api``.

This suite does NOT require a live feature store, warehouse, or schema.
It exists to enforce the CLI ↔ ``snowflake-ml-python[feature_store]``
contract: the real declarative library
(:mod:`snowflake.ml.feature_store.decl.api`) runs verbatim for the
*pure* paths — ``load_project`` / ``validate_specs`` / ``generate_plan``
/ ``serialize_plan`` / ``deserialize_plan`` / ``state_queries`` /
``fetch_applied_state`` / ``enrich_list_results`` — while everything
that would touch Snowflake is sealed:

* The connection (``execute_query`` / ``_build_session``) is mocked;
  ``execute_query`` returns empty iterators, modelling an empty
  deployed schema (empty applied state).
* The *imperative* seam — ``assert_feature_store_initialized`` and the
  ``fetch_*_rows`` facades that each construct a real
  ``FeatureStore(FAIL_IF_NOT_EXIST)`` — is stubbed by the autouse
  ``bypass_init_first_guard`` fixture, and ``forbid_live_feature_store``
  fails loudly if any path slips past it and tries to build a live
  ``FeatureStore``.
...
# A StreamingFeatureView is always backed by a StreamConfig, which cannot be
# built without a named stream source and a transformation UDF. Both are
# required at plan time (STREAM_FV_SOURCE_REQUIRED / STREAM_FV_UDF_REQUIRED),
# front-running the errors the imperative register flow would otherwise raise
# at apply.
...
# UDF body companion for ``_FV_YAML``'s ``udf.file``. The loader treats a ``.py``
# claimed by a sibling YAML's ``udf.file:`` as a function body, not a spec module.
_FV_UDF_PY = textwrap.dedent(
    """\
...
# The stream source ``_FV_YAML`` references. A source named by an FV must exist
# as a spec in the batch (or in applied state), else MISSING_SOURCE.
...
def _write_minimal_project(project_root: Path) -> Path:
    ...
    Tree::
        ...
        <project_root>/
          ...
          sources/
            ...
            feature_views/user_clicks.py    (UDF body claimed via udf.file)
    ...
    (sources / "datasources" / "clicks.yaml").write_text(_SOURCE_YAML)
    (sources / "feature_views" / "user_clicks.yaml").write_text(_FV_YAML)
    (sources / "feature_views" / "user_clicks.py").write_text(_FV_UDF_PY)
    return project_root
...
def bypass_init_first_guard():
    ...
    5. ``decl_api.fetch_stream_source_rows`` — covers the
       stream-source enumeration path threaded into
       ``_fetch_applied_state_bundle``; same reasoning as #2 / #3 / #4.

    Every ``decl_api.fetch_*`` facade constructs a real
    ``FeatureStore(FAIL_IF_NOT_EXIST)`` inside
    ``imperative_executor.py`` and runs the library's own init check,
    so patch #1 (which only intercepts the manager's CLI-direct calls)
    does not reach them. Any new ``FeatureStore``-constructing
    ``decl_api.fetch_*`` wired into the manager MUST be added here, or
    it will try to build a live ``FeatureStore`` against the mock
    session (the autouse ``forbid_live_feature_store`` net turns that
    into a clear contract failure).
    ...
    with mock.patch(
    ...
    ) as m_fetch_fgs, mock.patch(
        "snowflake.cli._plugins.feature.manager.decl_api.fetch_stream_source_rows"
    ) as m_fetch_sss:
        ...
        m_fetch_sss.return_value = []
        yield m_assert


@pytest.fixture(autouse=True)
def forbid_live_feature_store():
    """Fail loudly if any code path constructs a real ``FeatureStore``.

    This suite is an *offline* library-contract test: the whole point
    is that no ``FeatureStore(FAIL_IF_NOT_EXIST)`` is ever built against
    the mock session. ``bypass_init_first_guard`` stubs every known
    ``decl_api`` seam that would do so; this net catches the *next*
    forgotten one and turns it into a clear contract failure instead of
    an opaque warehouse / ``NOT_FOUND`` traceback from deep inside the
    library.

    ``TestInitIntegration`` legitimately drives the constructor (with
    ``creation_mode=CREATE_IF_NOT_EXIST``); those tests nest their own
    ``mock.patch`` of the same target, which wins over this autouse
    patch, so the bootstrap path stays exercised.
    """

    def _forbidden(*args, **kwargs):
        raise AssertionError(
            "live FeatureStore constructed — stub the decl_api "
            "fetch_* / assert seam in bypass_init_first_guard"
        )
...
class TestPlanIntegration:
    ...
    def test_plan_returns_ops_list(self, project_dir, mock_execute_query):
        ...
        assert result["executed"] == 0
        # Empty applied state → the real planner must emit CREATE ops for
        # the fixture's authored specs. This proves load_project +
        # generate_plan actually ran (not just returned an empty list).
        op_names = {str(op.get("name", "")).lower() for op in result["ops"]}
        assert {
            "user",
            "user_clicks",
        } <= op_names, (
            f"planner must emit ops for the authored entity/FV; got {op_names!r}"
        )
        assert all(
            str(op.get("operation", "")).upper().startswith("CREATE")
            for op in result["ops"]
        ), f"empty applied state should yield only CREATE ops; got {result['ops']!r}"
...
class TestWritePlanIntegration:
    ...
    def test_write_plan_creates_json_file(
        ...
        # Real serializer over a real plan against empty applied state:
        # the authored entity/FV must appear as CREATE ops in the envelope.
        op_names = {str(op.get("name", "")).lower() for op in parsed["plan"]["ops"]}
        assert {
            "user",
            "user_clicks",
        } <= op_names, (
            f"serialized plan must record ops for the authored specs; got {op_names!r}"
        )
