...
class TestApplySqlUsesConnectionContext:
    """SQL generated by apply should use the connection db/schema/warehouse."""

    def test_sql_contains_connection_database_and_schema(
        ...
        # The SQL should NOT have ".." (empty db/schema)
        ddl_calls = [
            c[0][0]
            for c in mock_execute_query.call_args_list
            if "CREATE ONLINE FEATURE TABLE" in str(c[0][0]).upper()
        ]
        assert ddl_calls, "Expected at least one CREATE ONLINE FEATURE TABLE DDL"
        for sql in ddl_calls:
            assert ".." not in sql, f"SQL has empty db/schema: {sql[:200]}"
            assert "MY_DB" in sql, f"SQL missing connection database: {sql[:200]}"
            assert "MY_SCHEMA" in sql, f"SQL missing connection schema: {sql[:200]}"
