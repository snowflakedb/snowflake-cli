...
@mock.patch(execute_queries)
def test_plan_project_with_delta(mock_execute_query, project_directory):
    mgr = DCMProjectManager()
    mgr.plan(
        project_identifier=TEST_PROJECT,
        from_stage="@my_stage",
...
@mock.patch(execute_queries)
def test_plan_project_with_delta_and_configuration(
    mock_execute_query, project_directory
):
    mgr = DCMProjectManager()
    mgr.plan(
        project_identifier=TEST_PROJECT,
    ...
    mock_execute_query.assert_called_once_with(
        query="EXECUTE DCM PROJECT IDENTIFIER('my_project') PLAN DELTA"
        " USING CONFIGURATION some_configuration (key=>value) FROM @my_stage"
    )
