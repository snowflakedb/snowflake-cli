...
def test_mixed_recursion(
    ...
    source = query_reader(f"select 0; !source {url}; select 0a;")
    errors, cnt, compiled = compile_statements(source, WORKING_OPERATOR_FUNCS)
...
def test_recursion_multiple_files(tmp_path_factory: pytest.TempPathFactory):
    ...
    source = file_reader((SecurePath(f1),))
    errors, cnt, compiled = compile_statements(source, WORKING_OPERATOR_FUNCS)
...
def test_source_missing_url(httpserver: HTTPServer):
    ...
    source = query_reader(f"!source {httpserver.url_for('missing.sql')};")
    errors, cnt, compiled = compile_statements(source, WORKING_OPERATOR_FUNCS)
    assert errors
...
def test_read_files(tmp_path_factory: pytest.TempPathFactory):
    ...
    errors, cnt, compiled = compile_statements(
        file_reader(files), WORKING_OPERATOR_FUNCS
    )
