...
class TestSclsSubmit:
    ...
    def test_submit_spark_application(self, mock_manager, runner, tmp_path):
        ...
        mock_manager().submit.assert_called_once_with(
            "EXECUTE SPARK APPLICATION ENVIRONMENT_RUNTIME_VERSION='0.1' STAGE_MOUNTS=('@my_stage/jars:/tmp/entrypoint') ENTRYPOINT_FILE='/tmp/entrypoint/test.jar' CLASS = 'com.example.Main' RESOURCE_CONSTRAINT='CPU_2X_X86'",
            None,
        )
    ...
    def test_submit_with_application_arguments(self, mock_manager, runner, tmp_path):
        ...
        assert result.exit_code == 0, result.output
        mock_manager().submit.assert_called_once_with(
            "EXECUTE SPARK APPLICATION ENVIRONMENT_RUNTIME_VERSION='0.1' STAGE_MOUNTS=('@stage:/tmp/entrypoint') ENTRYPOINT_FILE='/tmp/entrypoint/app.jar' CLASS = 'com.example.Main' ARGUMENTS = ('arg1','arg2') RESOURCE_CONSTRAINT='CPU_2X_X86'",
            None,
        )
    ...
    @mock.patch(SCLS_MANAGER)
    def test_submit_with_snow_secrets_option(self, mock_manager, runner, tmp_path):
        """Test submitting a Spark application with --snow-secrets option."""
        entrypoint = tmp_path / "app.py"

        mock_manager().upload_file_to_stage.return_value = "app.py"
        mock_manager().submit.return_value = (
            "Spark Application ID: app-with-snow-secrets"
        )
        ...
        result = runner.invoke(
            [
                ...
                "@my_stage",
                "--snow-secrets",
                "secret1=secret1_value,secret2=secret2_value",
            ]
        )
        assert result.exit_code == 0, result.output
        assert "Spark Application ID: app-with-snow-secrets" in result.output
        submit_query = mock_manager().submit.call_args[0][0]
        assert (
