...
class TestRemoteManagerSSH:
    ...
    def test_validate_ide_requirements_existing_service(
        self, mock_get_status, mock_resolve
    ):
        """Test IDE validation when using existing service."""
        manager = RemoteManager()

        mock_resolve.return_value = "resolved_service_name"
        mock_get_status.return_value = (True, "RUNNING")  # Service exists

        # Should not raise for existing service even without EAI
    ...
    @patch("snowflake.cli._plugins.remote.manager.connect_to_snowflake")
    ...
    @patch("snowflake.cli._plugins.remote.manager.RemoteManager.get_server_ui_url")
    @patch("snowflake.cli.api.console.cli_console.warning")
    def test_handle_existing_service_running_with_config_warning(
        self, mock_warning, mock_get_url
    ):
