...
class TestContainerSpec:
    ...
    def test_generate_spec_with_invalid_stage(self, mock_session):
        ...
        # Test with empty string after formatting (should still be invalid)
        with pytest.raises(
            ValueError,
            match="Invalid stage path: '@'. Stage path must start with '@' and contain a stage name",
        ):
            generate_service_spec(
                session=mock_session,
                compute_pool="test_pool",
                stage="",  # Empty string, will become "@" after formatting but still invalid
            )
