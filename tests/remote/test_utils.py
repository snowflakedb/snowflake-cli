...
class TestSSHSecurityFixes:
    """Test SSH security-related fixes."""
    ...
    def test_generate_ssh_config_lines_shell_escaping(self):
        ...
        from snowflake.cli._plugins.remote.utils import _generate_ssh_config_lines
        ...
        # Verify the token is properly escaped (should be single-quoted)
        ...
        # Verify private key path is used as-is (no quoting needed per user's change)
        assert identity_file_line is not None
        assert "/path with spaces/key" in identity_file_line

    def test_generate_ssh_config_lines_normal_values(self):
        """Test that normal values still work correctly with escaping."""
        from snowflake.cli._plugins.remote.utils import _generate_ssh_config_lines

        config_lines = _generate_ssh_config_lines(
            service_name="test_service",
            hostname="example.com",
            websocat_path="/usr/bin/websocat",
        ...
        # Normal values should still work (may or may not be quoted depending on content)
        assert proxy_command_line is not None
        assert "Authorization: Snowflake Token=" in proxy_command_line
        assert "normal_token_123" in proxy_command_line

    def test_generate_ssh_config_lines_path_without_spaces(self):
        """Test that paths without spaces are not unnecessarily quoted."""
        from snowflake.cli._plugins.remote.utils import _generate_ssh_config_lines
        ...
        config_lines = _generate_ssh_config_lines(
            ...
            websocat_path="/usr/bin/websocat",
            token="normal_token",
            private_key_path="/home/user/.ssh/id_rsa",  # No spaces
        )

        # Find the IdentityFile line
        identity_file_line = None
        for line in config_lines:
        ...
        assert identity_file_line is not None
        assert "IdentityFile /home/user/.ssh/id_rsa" in identity_file_line
        assert '"' not in identity_file_line  # No quotes should be present
