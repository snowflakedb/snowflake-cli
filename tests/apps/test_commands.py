...
class TestDeployCommand:
    ...
    def test_deploy_drops_stage_after_build_by_default(
        self,
        mock_resolve,
        mock_get_entity,
        mock_defaults,
        ...
        """Default deploy (no --keep-code-stage) drops the stage after build."""
        from snowflake.cli.api.project.project_paths import ProjectPaths
        ...
        mock_mgr.drop_stage.assert_called_once_with(
            FQN(database="TEST_DB", schema="TEST_SCHEMA", name="MY_APP_CODE_STAGE")
        )
    ...
    def test_deploy_keep_code_stage_skips_drop(
        self,
        mock_resolve,
        mock_get_entity,
        mock_defaults,
        ...
        """--keep-code-stage prevents the stage from being dropped."""
        from snowflake.cli.api.project.project_paths import ProjectPaths
        ...
        with with_feature_flags({FeatureFlag.ENABLE_SNOWFLAKE_APPS: True}):
            ...
            with change_directory(tmp_path):
                result = runner.invoke(["__app", "deploy", "--keep-code-stage"])
                assert result.exit_code == 0, result.output

        mock_mgr.drop_stage.assert_not_called()
    ...
    def test_deploy_drops_stage_even_on_build_failure(
        self,
        mock_resolve,
        mock_get_entity,
        mock_defaults,
        ...
        """Stage is dropped even when the build fails."""
        from snowflake.cli.api.project.project_paths import ProjectPaths
        ...
        mock_mgr.execute_build_job.side_effect = CliError("build exploded")
