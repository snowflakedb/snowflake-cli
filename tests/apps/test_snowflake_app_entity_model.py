...
class TestSnowflakeAppEntityModel:
    ...
    def test_compute_resource_defaults_to_none(self):
        """``compute_resource`` is optional and defaults to None."""
        model = SnowflakeAppEntityModel(
            type="snowflake-app",
            identifier="my_app",
            artifacts=["app/*"],
        )
        assert model.compute_resource is None

    @pytest.mark.parametrize(
        "value, expected",
        [
            ("SERVERLESS", "SERVERLESS"),
            ("MANAGED_COMPUTE_POOL", "MANAGED_COMPUTE_POOL"),
            ("serverless", "SERVERLESS"),
            ("  managed_compute_pool  ", "MANAGED_COMPUTE_POOL"),
        ],
    )
    def test_compute_resource_normalised(self, value, expected):
        """Accepted values are normalised to their upper-case DDL form."""
        model = SnowflakeAppEntityModel(
            type="snowflake-app",
            identifier="my_app",
            artifacts=["app/*"],
            compute_resource=value,
        )
        assert model.compute_resource == expected

    @pytest.mark.parametrize("value", [None, "null"])
    def test_compute_resource_none_values(self, value):
        """``compute_resource`` accepts None and 'null' as None."""
        model = SnowflakeAppEntityModel(
            type="snowflake-app",
            identifier="my_app",
            artifacts=["app/*"],
    ...
    def test_compute_resource_invalid_value_rejected(self):
        """An unrecognised ``compute_resource`` value raises a validation error."""
        with pytest.raises(ValueError, match="compute_resource must be one of"):
            SnowflakeAppEntityModel(
                type="snowflake-app",
                identifier="my_app",
                artifacts=["app/*"],
                compute_resource="TURBO",
            )

    @pytest.mark.parametrize("value", [42, 3.14, True, ["SERVERLESS"]])
    def test_compute_resource_non_string_rejected(self, value):
        """A non-string ``compute_resource`` is rejected before normalisation."""
        with pytest.raises(ValueError, match="compute_resource must be a string"):
            SnowflakeAppEntityModel(
                type="snowflake-app",
                identifier="my_app",
                artifacts=["app/*"],
    ...
    def test_compute_resource_hidden_from_json_schema(self):
        """``compute_resource`` is a provisional field hidden from the schema."""
        properties = SnowflakeAppEntityModel.model_json_schema()["properties"]
        assert "compute_resource" not in properties
