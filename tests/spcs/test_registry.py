...
def test_get_registry_url_no_repositories_cli(
    mock_get_registry_url, runner, custom_snapshot
):
...
def test_docker_registry_login_cli(mock_docker_login, runner, custom_snapshot):
...
def test_docker_registry_login_subprocess_error(
    mock_check_output, mock_get_url, mock_get_token, custom_snapshot
):
    test_url = (
        mock_get_url.return_value
    ) = "orgname-acctname.registry.snowflakecomputing.com"
    ...
    assert e.value.message == custom_snapshot
