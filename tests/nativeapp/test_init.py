...
def test_nativeapp_init_with_default_template_and_repo(
    mock_clone_from, temp_dir, custom_snapshot
):
    ...
    assert project_path.joinpath("snowflake.yml").read_text() == custom_snapshot
...
def test_nativeapp_init_with_custom_repo(mock_clone_from, temp_dir, custom_snapshot):
    project = nativeapp_init(
