...
@mock.patch(SQL_EXECUTOR_EXECUTE_QUERIES)
def test_execute_catches_no_warehouse_error(mock_execute_queries):
    # Arrange
    mock_script = "-- my comment\nselect 1;\nselect 2;"
    mock_script_name = "test-user-sql-script.sql"
...
@mock.patch(SQL_EXECUTOR_EXECUTE_QUERIES)
def test_execute_catches_other_programming_error(mock_execute_queries):
    # Arrange
    mock_script = "-- my comment\nselect 1;\nselect 2;"
    mock_script_name = "test-user-sql-script.sql"
    mock_execute_queries.side_effect = ProgrammingError()
...
@mock.patch(SQL_EXECUTOR_EXECUTE_QUERIES)
def test_execute_catches_all_exception(mock_execute_queries):
    # Arrange
    mock_script = "-- my comment\nselect 1;\nselect 2;"
    mock_script_name = "test-user-sql-script.sql"
    mock_execute_queries.side_effect = Exception()
...
def test_use_object_catches_other_programming_error(mock_execute_query):
    ...
    with pytest.raises(ProgrammingError) as err:
        sql_facade._use_object(object_type, object_name)  # noqa: SLF001
    assert err.value.msg == "Failed to use warehouse test_warehouse"
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            Exception,
            UnknownSQLError,
            "Unknown SQL error occurred. Failed to use warehouse test_warehouse",
        ),
    ],
)
@mock.patch(SQL_EXECUTOR_EXECUTE)
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            Exception,
            UnknownSQLError,
            "Unknown SQL error occurred. Failed to use role test_role",
        ),
    ],
)
@mock.patch(SQL_EXECUTOR_EXECUTE)
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            Exception,
            UnknownSQLError,
            "Unknown SQL error occurred. Failed to use database test_db",
        ),
    ],
)
@mock.patch(SQL_EXECUTOR_EXECUTE)
