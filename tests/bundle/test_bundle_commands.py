...
def test_history_help(runner):
    ...
    assert "--execution-name" in result.output
...
def test_history_passes_result_limit(mock_history, runner, mock_cursor):
    ...
    result = runner.invoke(["bundle", "history", "--result-limit", "50"])
...
@mock.patch.object(CodeBundleManager, "history")
def test_history_calls_manager_with_options(mock_history, runner, mock_cursor):
    mock_history.return_value = mock_cursor(rows=[], columns=[])
...
def test_history_query_with_result_limit(mock_connector, mock_ctx, runner):
    ...
    result = runner.invoke(["bundle", "history", "--result-limit", "5"])
...
@mock.patch("snowflake.connector.connect")
def test_history_query_with_bundle_name(mock_connector, mock_ctx, runner):
    ctx = mock_ctx()
    mock_connector.return_value = ctx

    result = runner.invoke(["bundle", "history", "--bundle-name", "my_bundle"])
...
@mock.patch("snowflake.connector.connect")
def test_history_query_with_database(mock_connector, mock_ctx, runner):
    ctx = mock_ctx()
    mock_connector.return_value = ctx

    result = runner.invoke(["bundle", "history", "--bundle-database", "my_db"])
...
@mock.patch("snowflake.connector.connect")
def test_history_query_with_schema(mock_connector, mock_ctx, runner):
    ctx = mock_ctx()
    mock_connector.return_value = ctx

    result = runner.invoke(["bundle", "history", "--bundle-schema", "my_schema"])
...
@mock.patch("snowflake.connector.connect")
def test_history_query_with_execution_name(mock_connector, mock_ctx, runner):
    ctx = mock_ctx()
    mock_connector.return_value = ctx

    result = runner.invoke(["bundle", "history", "--execution-name", "my_execution"])
...
def test_history_query_escapes_quote(mock_connector, mock_ctx, runner):
    ...
    result = runner.invoke(["bundle", "history", "--bundle-name", "it's_bundle"])

    assert result.exit_code == 0, result.output
    assert ctx.get_query() == (
        "SELECT * FROM TABLE("
        "SNOWFLAKE.INFORMATION_SCHEMA.CODE_BUNDLE_HISTORY("
        "BUNDLE_NAME => 'it''s_bundle', "
        "RESULT_LIMIT => 100))"
    )


def test_history_no_args_succeeds(runner, mock_cursor):
    with mock.patch.object(CodeBundleManager, "history") as mock_history:
        mock_history.return_value = mock_cursor(rows=[], columns=[])
        result = runner.invoke(["bundle", "history"])
