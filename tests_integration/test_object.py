...
def test_object_table(runner, test_database, snowflake_session):
    object_name = ObjectNameProvider("Test_Object_Table").create_and_get_next_object_name()
    ...
    actual_tables = row_from_cursor(snowflake_session.execute_string(f"show tables")[-1])
    assert result_show.json[0].keys()== actual_tables[0].keys()
    assert result_show.json[0]["name"].lower() == object_name.lower()

    result_describe = runner.invoke_integration(["object","describe","table", object_name])
    ...
    assert result_drop.json[0]["status"].lower() == f"{object_name.lower()} successfully dropped."
    assert len(row_from_cursor(snowflake_session.execute_string(f"show tables")[-1])) == 0
