...
def test_stage(mock_print, runner, snowflake_session, tmp_path):
    ...
    runner.invoke_with_config(["stage", "create", stage_name])
    ...
    with NamedTemporaryFile("w+", suffix=".txt") as tmp_file:
        runner.invoke_with_config(["stage", "put", tmp_file.name, stage_name])
    ...
    runner.invoke_with_config(["stage", "remove", stage_name, f"/{filename}"])
    assert contains_row_with(
        row_from_mock(mock_print),
    ...
    runner.invoke_with_config(["stage", "drop", stage_name])
    assert contains_row_with(
        row_from_mock(mock_print),
