...
* Fixed `snow app deploy` failing with connector error `253006` when uploading app code whose paths contain glob metacharacters — for example Next.js dynamic-route directories such as `[id]` or `[...slug]`. Local file paths are now escaped before being passed to `PUT`, so the connector matches the literal file instead of interpreting `[id]` as a pattern (which matched nothing, or a same-named sibling directory). This affected both the workspace and stage upload paths.
* Fixed `snow app deploy` failing with connector error `253006` (`Not a file but a directory`) when uploading app code to a stage for a project whose bundle contained subdirectories. The stage upload now sends each file with its own `PUT` (preserving the directory structure) instead of a recursive `PUT <dir>/*` glob that also matched directories, and no longer deletes the local bundle while uploading.
