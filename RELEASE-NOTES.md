...
## New additions
* Added `snow dcm init` command that scaffolds or extends a DCM Project. You either create a new project — pass `--project-name` (or choose a name interactively) and a subfolder is created with a `manifest.yml` and a `sources/definitions/raw.sql` placeholder inside `./<name>/` — or add a new target to an existing `manifest.yml` in the current directory (leaving its `default_target` unchanged and reusing the existing definition files). You then name the target (with `--target`; defaults to the current account alias, re-prompting if the name is invalid or already taken) and the DCM Project object (with `--project-identifier`; defaults to the target name, with special characters automatically double-quoted so the identifier stays valid instead of failing). If the object name is not fully qualified, the connection's default database and schema are used or you are prompted for them, and if the current role cannot be determined you are prompted for the role that will own the DCM Project (rather than recording a placeholder). The generated manifest stores a fully-qualified `project_name` and an `account_identifier` in `ORG-LOCATOR` form. All changes — creating the project structure, creating Snowflake objects (warehouse, database, schema, DCM Project), or editing an existing manifest — are summarized and confirmed up front, so nothing is created if you decline; pass `--force` to approve them non-interactively (which then requires `--target`, and reuses an already-existing target instead of failing). If the connection has no warehouse, `snow dcm init` provisions one (an X-Small `DCM_WH` by default) and prints how to configure it for subsequent `plan`/`deploy` commands. After initialization, it prints the next steps, including how to run `snow dcm plan`/`snow dcm deploy` for the new target.
