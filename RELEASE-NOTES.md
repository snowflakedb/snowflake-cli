...
* DCM projects: `snow dcm deploy`, `snow dcm plan`, and `snow dcm preview` now support environment variables and secrets declared in a project manifest's `templating.env_vars`/`templating.env_secrets` sections. Declared values are collected from the shell environment at command time and forwarded to the server.
