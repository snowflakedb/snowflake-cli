...
* `snow bundle` manages Snowflake code bundles for notebook projects. `snow bundle create` takes `--source` as a stage path (`@stage/path`), a workspace path (`snow://workspace/...`), or a local directory (`file://` or no protocol prefix) whose tree is uploaded to a temporary stage first; `--exclude` takes glob patterns matched against each path component, so `venv` skips a directory of that name at any depth and `*.pyc` skips those files anywhere. `--comment` sets a comment, and `--overwrite` or `--skip-if-exists` decide what happens when the bundle already exists. `snow bundle list` filters with `--like`, `--in database|schema <name>`, and `--in-account`; `snow bundle delete` accepts `--if-exists`; `snow bundle alter` takes exactly one of `--rename-to` or `--add-version <source>`. `snow bundle execute` runs a bundle at `--entrypoint`, labels the run with `--execution-name`, and forwards any arguments given after the known options to the bundle itself; with `--async` it returns the query ID immediately for `snow bundle status <query-id>` and `snow bundle cancel <query-id>`. `snow bundle history` lists executions and filters them by `--bundle-name`, `--bundle-database`, `--bundle-schema`, `--entrypoint`, `--start-time-range-start` / `--start-time-range-end`, `--bundle-types`, `--compute-types`, `--language-types`, `--status`, and `--execution-name`, with `--result-limit` capping the rows returned (default 100).
