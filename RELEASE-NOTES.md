...
* Fixed SQL injection via `FQN.sql_identifier`: single quotes in identifiers are now escaped using `to_string_literal`, which also handles the `\'` bypass where the connector's client-side statement splitter treated `\'` as an escape pair, allowing a second statement to be injected. (SNOW-3392936)
