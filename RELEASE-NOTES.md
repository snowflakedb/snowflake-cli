...
* Added `--dbt-version` flag to `snow dbt deploy` and `snow dbt execute` commands. This flag allows to set dbt Core version on dbt project object (`deploy` command) or execute a dbt command on a specific dbt version, without altering the dbt object (`execute` commands).
