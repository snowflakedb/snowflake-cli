...
## New additions
* (PrPr, gated on `SNOWFLAKE_CLI_FEATURES_ENABLE_DCM_EARLY_ACCESS=true`) Added `snow dcm analyze` command. It runs the same analysis as the internal `snow dcm raw-analyze` (`EXECUTE DCM PROJECT ... ANALYZE`) but prints a formatted list of findings instead of the raw JSON payload. The reporter consumes the server's `issues[]` arrays at the top, file, and per-definition levels and color-codes each finding by its `severity` field: `ERROR` is red, `WARN`/`WARNING` is yellow, and `INFO` is plain blue. Unknown or missing severity values are treated as `ERROR` so a buggy server payload never silently downgrades a real problem. Each finding is printed as its cleaned message text, grouped under the source file and (when applicable) the fully-qualified definition identifier and domain; the server-side `DCM project ANALYZE error: ` prefix is stripped from messages. The client-side UPLOAD phase is rendered with the same live progress styling as `snow dcm deploy`/`snow dcm plan` (`UPLOAD` row with progress bar, completion checkmark, timestamp, duration, and dim per-folder upload summaries). A summary line is always printed at the end (similar to `snow dcm plan`): a green `Static analysis of DCM Project files found no errors.` when nothing was reported, or `Static analysis of DCM Project files found N errors, M warnings, and K infos.` (with each severity count color-coded; zero buckets are omitted) otherwise. Source file paths in the per-file findings list are rendered in bold blue for visibility. Supports `--save-output` to persist the raw JSON response under `out/analyze.json` and download server-side artifacts to `out/analyze/`, matching the behavior of `snow dcm plan`. Exits with status 1 only when severity=ERROR findings are present; warning/info-only runs exit 0.
