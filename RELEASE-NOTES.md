...
* `snow dcm test` now renders its styled `N passed, M failed out of T total.` summary above the `===` divider on every run (including when one or more expectations fail), matching the new presentation already used by `snow dcm analyze` and `snow dcm refresh`. The summary text is no longer wrapped inside an error box on failure; failures continue to exit with status 1.
