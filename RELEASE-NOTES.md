...
## New additions
* `snow dcm deploy` and `snow dcm plan` now render a live phase checklist while the command runs, replacing the prior single-line spinner. Each phase (UPLOAD → RENDER → COMPILE → PLAN → DEPLOY for deploy; UPLOAD → RENDER → COMPILE → PLAN for plan) shows its own row with a colored status indicator: an animated braille spinner while running, a green check on completion (with start timestamp and elapsed duration), or a red ✗ on failure. PLAN and DEPLOY additionally show a 0–100 % progress bar derived from `SYSTEM$GET_DCM_PROJECT_PROGRESS`. The UPLOAD row also reports its own progress bar driven by the actual file-upload count, with the stage-creation message and per-folder upload counts rendered dim-indented beneath it. Polling is adaptive (1 s for the first 100 s, then 10 s) to keep cost low on long-running deploys. The Live UI is automatically suppressed in `--silent` mode and in `--format json` / `--format csv` runs so machine-readable output stays clean.
* `snow dcm plan` and `snow dcm deploy` now expand `ALTER` rows with their nested sub-changes. Each entity-level `ALTER` row is followed by indented detail lines describing every leaf change (added / removed / modified / renamed / set / unset) inferred from the server's nested `changes[]` payload, with `collection`-kind wrappers (e.g. `grants`, `data_metric_functions`, `privileges`) inlined. Sub-changes are rendered as a Unicode box-drawing tree (`├─`, `└─`, `│`) under each `ALTER` row and sorted by category — all `added` first, then `modified`, then `removed` — mirroring the top-level CREATE → ALTER → DROP ordering. Only the operation keyword is colored (added/set in green, modified/renamed in yellow, removed/unset in red); the description text renders in the terminal default for readability. `CREATE` and `DROP` rows continue to render as a single line. The fallback parser still surfaces best-effort details when the server returns a payload that does not validate against the strict schema.
* (PrPr, gated on `SNOWFLAKE_CLI_FEATURES_ENABLE_DCM_EARLY_ACCESS=true`) Added `snow dcm analyze` command. It runs the same analysis as the internal `snow dcm raw-analyze` (`EXECUTE DCM PROJECT ... ANALYZE`) but prints a formatted list of findings instead of the raw JSON payload. The reporter consumes the server's `issues[]` arrays at the top, file, and per-definition levels and color-codes each finding by its `severity` field: `ERROR` is red, `WARN`/`WARNING` is yellow, and `INFO` is plain blue. Unknown or missing severity values are treated as `ERROR` so a buggy server payload never silently downgrades a real problem. Each finding is printed as its cleaned message text, grouped under the source file and (when applicable) the fully-qualified definition identifier and domain; the server-side `DCM project ANALYZE error: ` prefix is stripped from messages. The client-side UPLOAD phase plus a server-side ANALYZE phase are rendered with the same live progress styling as `snow dcm deploy`/`snow dcm plan` (animated spinner next to the running phase; UPLOAD shows a progress bar). A summary line is always printed at the end (similar to `snow dcm plan`): a green `Static analysis of DCM Project files found no errors.` when nothing was reported, or `Static analysis of DCM Project files found N errors, M warnings, and K infos.` (with each severity count color-coded; zero buckets are omitted) otherwise. Source file paths in the per-file findings list are rendered in bold blue for visibility. Supports `--save-output` to persist the raw JSON response under `out/analyze.json` and download server-side artifacts to `out/analyze/`, matching the behavior of `snow dcm plan`. Exits with status 1 only when severity=ERROR findings are present; warning/info-only runs exit 0.
...
* `snow dcm test` now prints a styled `Ran T data quality tests: N passed, M failed expectation(s).` summary above the `===` divider on every run (including when one or more expectations fail), matching the new presentation already used by `snow dcm analyze` and `snow dcm refresh`. The trailing noun (`expectation` / `expectations`) pluralizes with the last visible bucket — `unknown` when present, otherwise `failed`. The summary text is no longer wrapped inside an error box on failure; failures continue to exit with status 1.
