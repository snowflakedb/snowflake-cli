...
## New additions
* (PrPr, gated on `SNOWFLAKE_CLI_FEATURES_ENABLE_DCM_EARLY_ACCESS=true`) Added `snow dcm analyze` command. It runs the same analysis as the internal `snow dcm raw-analyze` (`EXECUTE DCM PROJECT ... ANALYZE`) but prints a formatted list of findings instead of the raw JSON payload: `errors` are rendered in red, `issues` are rendered in yellow as warnings. Each finding is printed as its cleaned message text, grouped under the source file and (when applicable) the fully-qualified definition identifier and domain; the server-side `DCM project ANALYZE error: ` prefix is stripped from messages. The client-side UPLOAD phase is rendered with the same live progress styling as `snow dcm deploy`/`snow dcm plan` (`UPLOAD` row with progress bar, completion checkmark, timestamp, duration, and dim per-folder upload summaries). A summary line is always printed at the end (similar to `snow dcm plan`): a green `Static analysis of DCM Project files found no errors.` when nothing was reported, or `Static analysis of DCM Project files found N errors and M issues.` (with the error and issue counts color-coded) otherwise. Source file paths in the per-file findings list are now rendered in bold blue for visibility. Supports `--save-output` to persist the raw JSON response under `out/analyze.json` and download server-side artifacts to `out/analyze/`, matching the behavior of `snow dcm plan`. Exits with status 1 only when errors are found; warning-only runs exit 0.
