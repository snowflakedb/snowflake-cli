...
* Fixed `snow app` commands (e.g. `snow app deploy`, `snow app validate`) failing on Windows with a `UnicodeDecodeError` when `snowflake.yml` contained non-ASCII characters (e.g. a non-Latin app title or description). The `snow app` command group now reads and writes `snowflake.yml` as UTF-8 instead of falling back to the platform default code page (cp1252/cp932 on Windows). This UTF-8 handling is scoped to `snow app` commands; other commands continue to honor the `cli.encoding.file_io` setting / platform default when reading project files.
