...
* Improved the warning emitted when `default_connection_name` is set at the top of `connections.toml` (a common confusion — the key only belongs in `config.toml`). Both the legacy and `SNOWFLAKE_CLI_CONFIG_V2_ENABLED=1` paths now print: `'default_connection_name' is not a valid setting in connections.toml. Move it to config.toml (top-level) to set the default connection.` Previously the legacy path pointed users at the wrong fix (`[connections.default_connection_name]`) and the v2 path silently dropped the value.
