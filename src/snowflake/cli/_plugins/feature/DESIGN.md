...
`[cli.features] enable_feature_store = true` in `config.toml`. Every
invocation emits a yellow `WARNING: 'snow feature' is in public preview.`
banner on stderr (kept off structured stdout).

All feature-store logic is provided by the
`snowflake-ml-python[feature_store]` library (imported as
`snowflake.ml.feature_store.decl.*`). The library is an optional runtime
dependency: when it is absent the plugin still imports and registers, and
the group callback fails fast with
...
| `snow feature apply`    | `apply()`            | Pure plan-file consumer (auto-discovers latest `feature_plan_*.json` under `<project_root>/out/plan/` or honours `--plan`). Refuses to run unless BOTH (a) the target's `account_identifier` matches `get_account_identifier(connection)` and (b) the plan envelope's `target_name` matches `--target`. |
...
3. Asserts `AccountIdentifier.from_string(target.account_identifier)
   == get_account_identifier(connection)`.
   On mismatch, returns `target_mismatch` to the caller before any
   state SQL runs — the operator picks a different connection or
   fixes the manifest.

`_resolve_project` returns `(FSProjectPaths, FSManifest, TargetContext)`.  The caller threads the target's `database` / `schema` / `role` through every downstream `decl_api.*` call; `warehouse` is read from `ctx.connection.warehouse` (plan files are warehouse-agnostic by design).
...
It imports the shared library at module level behind a single try/except
guard so the plugin still imports (and registers) when
`snowflake-ml-python[feature_store]` is not installed:

```python
try:
    from snowflake.ml.feature_store.decl import api as decl_api
    from snowflake.ml.feature_store.decl.manifest import (
        FSManifest, FSTarget, ManifestNotFoundError,
    )
    from snowflake.ml.feature_store.decl.project_paths import FSProjectPaths
except ImportError:
    decl_api = None
    FSManifest = FSTarget = ManifestNotFoundError = FSProjectPaths = None
```

When the library is missing, the group callback in `commands.py`
(`_require_snowflake_ml`) raises an actionable `ClickException` before any
subcommand runs, so a command never reaches a `None` `decl_api`. When the
library is present, the manager delegates all business logic to
`decl_api.*`.

### No session priming

The CLI does not issue an `ALTER SESSION` to enable
`ENABLE_FEATURE_STORE_DESCRIBE_OFT_SPECIFICATION` — the parameter is
enabled by default at the account level. Every Snowflake-bound entry
point goes straight to its state SQL after the manifest resolution +
init-first guard, with no client-side session priming.
...
1. paths, manifest, target = self._resolve_project(from_dir, target_name)
   — manifest discovery + account-match assertion
...
4. Verify plan.target_name == target.name (name-strict)
...
6. On success, rename plan_file → plan_file + ".applied"
7. On failure, leave plan_file untouched so the operator can retry
8. Return result dict
```

`status` values returned to the CLI:

- `"applied"`  — every op executed successfully; plan file renamed `.applied`.
- `"refused"`  — the plan carried at least one `destructive=True` op
  and `--allow-recreate` was not set; **no op was executed**, and the
  plan file stays unrenamed so a follow-up
...
- `"target_mismatch"`  — either (a) the manifest target's
  ...
  or (b) the plan envelope's `target_name` differs from the
  requested `--target`; plan file untouched.
...
is enabled by default at the account level, so the CLI issues no
client-side `ALTER SESSION`.
...
`DESCRIBE … TYPE = SPECIFICATION` template out, hands the DT text to
the library to recover BatchFV sources and advanced fields, and
surfaces offline-only BFVs.  The `applied_state` kwarg
...
### `_fetch_stream_source_rows` helper
...
2. **Graceful-degrade-on-permission-error.** Any other exception (most commonly a privilege gap on `SYSTEM$LIST_FEATURE_STORE_OBJECTS('STREAM_SOURCE')`) is debug-logged and downgraded to an empty list. The downstream `fetch_applied_state` merge treats `stream_source_rows=[]` identically to `stream_source_rows=None` (FV-derived-only), so a missing privilege downgrades fidelity rather than crashing the bundle. **Exception — `_fetch_entity_rows` does NOT graceful-degrade.** The entity read is load-bearing for planner correctness: an empty applied-state entity set makes `validate_specs` emit `MISSING_ENTITY` for every FV referencing an applied-state-only entity. So `_fetch_entity_rows` re-raises any surviving backend error as a `CliError` (the imperative layer already retried a transient `list_entities()`/warehouse-auto-suspend loss) rather than returning `[]` — "unknown" must never be reported as "no entities registered".
...
Both methods load the runtime rows alongside the existing entity / FV / FG rows and forward them through `decl_api.fetch_applied_state(stream_source_rows=...)`. The two call sites are intentionally equivalent so the UI op stream and the disk plan see the same applied-state snapshot:

```python
# manager.plan(...) — UI op stream
...
- `begin_phase(label)` also finalizes the prior phase into `completed_base`
  ...
  driven by the `decl_api.fetch_*` facades' optional `on_progress`
  callback.

The library stays silent: it only *invokes* the
