...
class CortexManager(SqlExecutionMixin):
    def complete(
        ...
        escaped_model = self._escape_input(str(model))
    ...
    def translate_text(
        ...
        escaped_source = self._escape_input(
            str(source_language) if source_language else ""
        )
        escaped_target = self._escape_input(str(target_language))
        query = f"""\
