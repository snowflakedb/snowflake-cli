...
class AnalyzeReporter(Reporter[Dict[str, Any]]):
    ...
    def process(self, cursor: SnowflakeCursor) -> None:
        ...
        try:
            self._raw_data = row[0]
            result_json = json.loads(self._raw_data)
        except IndexError:
    ...
    def extract_data(self, result_json: Dict[str, Any]) -> List[Dict[str, Any]]:
        if not isinstance(result_json, dict):
            log.debug(
                "Unexpected response format. Expected response to be dictionary: %s",
                result_json,
            )
            raise CliError("Could not process response.")
    ...
    def print_renderables(self, data: Iterator[Dict[str, Any]]) -> None:
        ...
        if self._raw_data is not None:
            cli_console.styled_message(self._raw_data)
            cli_console.styled_message("\n")
