...
class _ChecklistProgress(Progress):
    ...
    def get_renderable(self) -> RenderableType:
        return Group(Text(""), *(self._render_row(task) for task in self.tasks))
...
class MultiStepProgress:
    ...
    alone otherwise), and elapsed time.
    ...
    def __init__(self, steps: Iterable[StepDefinition]) -> None:
        ...
        for step in steps:
            task_id = self._progress.add_task(
                "", label=step.label, state=StepState.PENDING, total=None, start=False
            )
    ...
    def _print_step_line(self, key: str, verb: str) -> None:
        if not self._display_open or self._is_tty:
            return
