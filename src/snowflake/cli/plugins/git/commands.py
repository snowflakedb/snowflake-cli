...
def create_app():
    ...
    def _assure_repository_does_not_exist(
        om: ObjectManager, repository_name: str
    ) -> None:
        if om.object_exists(
    ...
    def setup(
        ...
        * API integration - object allowing Snowflake to interact with git repository.
        ...
        if not om.object_exists(
            ...
            cli_console.step(
                f"API integration '{api_integration}' successfully created."
            )
        else:
    ...
    def list_branches(
        ...
        **options,
        ...
        """
        return QueryResult(
            GitManager().show_branches(repo_name=repository_name, like=like)
        )
