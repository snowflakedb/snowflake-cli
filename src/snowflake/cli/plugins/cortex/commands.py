...
def create_app():
    ...
    def search(
        ...
        limit: int = typer.Option(
            help="Maximum number of results retrieved", default=1
        ),
        **options,
    ):
        """
    ...
    def complete(
        ...
        **options,
        ...
        if text and file:
            raise UsageError(
                "--file option cannot be used together with TEXT argument."
            )
        if text:
            ...
            raise UsageError(
                "Either --file option or TEXT argument has to be provided."
            )
