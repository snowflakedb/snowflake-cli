...
from snowflake.cli.plugins.snowpark.package import commands as package_commands
...
def create_app():
    ...
    app.add_typer(package_commands.create_app())
    ...
    def deploy(
        ...
        **options,
        ...
        existing_procedures = _find_existing_objects(
            ObjectType.PROCEDURE, procedures, om
        )
        ...
        **options,
        ...
        with SecurePath.temporary_directory() as packages_dir:
            if snowpark_paths.defined_requirements_file.exists():
                ...
                log.info(
                    "Checking to see if packages have shared (.so/.dll) libraries..."
                )
